...
//        File file = new File("education.xlsx");
//        Workbook workbook = getWorkBook(file);
//        CellStyle cellStyle = workbook.createCellStyle();
//        cellStyle.setFillForegroundColor(IndexedColors.RED.getIndex());
//        cellStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
//
//        ExcelImportUtil.importExcelMore(file, EducationTrainExcelDto.class, getImportParams());
//        File localFile = new File("output.xlsx");
...
//           char[] charArrayS = s.substring(i, i + pLen).toCharArray();
//            Arrays.sort(charArrayS);
//            if (Arrays.equals(charArrayP, charArrayS)){
//                ret.add(i);
//            }
//        }
//        return ret;
//    }
    ...
    public int lengthOfLongestSubstring(String s) {
        int length = s.length(), max = 0, start = 0;
        Map<Character, Integer> map = new HashMap<>();
        for (int i = 0; i < length; i++){
            char charAt = s.charAt(i);
            if (map.containsKey(charAt)){
                start = Math.max(start, map.get(charAt));
            }
            ...
            if (first == null || nums[i] != first){
                ...
                for (int j = i+1; j < nums.length; j++){
                    if (second == null || second != nums[j]){
                        ...
                        if (second + nums[third] == target){
                            result.add(Arrays.asList(nums[i], second, nums[third]));
//                            List<Integer> item = Arrays.asList(nums[i], second, nums[third]);
...
//    }
    ...
    public static List<List<String>> groupAnagrams(String[] strs) {
        ...
        for (int i= 0; i < strs.length; i++){
            ...
            List<String> stringList = map.getOrDefault(valueOf, new ArrayList<>());
            stringList.add(strs[i]);
            map.put(valueOf, stringList);
        }

        return new ArrayList<>(map.values());
    }

    private static ImportParams getImportParams() {
